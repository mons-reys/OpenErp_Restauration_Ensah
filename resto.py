...
class resto_order(osv.Model):
    """ 
    resto order (contains one or more resto order line(s))
    """
    _name = 'resto.order'
    ...
    def _price_get(self, cr, uid, ids, name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            result[order.id] = sum(order_line.meal_id.price
    ...
    def add_preference(self, cr, uid, ids, pref_id, context=None):
        ...
        """
        assert len(ids) == 1
        orderline_ref = self.pool.get('resto.order.line')
        prod_ref = self.pool.get('resto.meal')
        order = self.browse(cr, uid, ids[0], context=context)
        pref = orderline_ref.browse(cr, uid, pref_id, context=context)
        new_order_line = {
            'date': order.date,
            'user_id': uid,
            'meal_id': pref.meal_id.id,
            'note': pref.note,
            'order_id': order.id,
            'price': pref.meal_id.price,
            'supplier': pref.meal_id.supplier.id
        }
    ...
    def _default_alerts_get(self, cr, uid, context=None):
        ...
        get the alerts to display on the order form
        """
        alert_ref = self.pool.get('resto.alert')
    ...
    def onchange_price(self, cr, uid, ids, order_line_ids, context=None):
        ...
        if order_line_ids:
            tot = 0.0
            meal_ref = self.pool.get("resto.meal")
            for prod in order_line_ids:
                if 'meal_id' in prod:
                    tot += meal_ref.browse(cr, uid, prod['meal_id'], context=context).price
                else:
                    tot += prod['price']
            res = {'value': {'total': tot}}
        return res
    ...
    def __getattr__(self, attr):
        ...
        return super(resto_order,self).__getattr__(self,attr)

    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        """ 
        Add preferences in the form view of order.line 
        """
        res = super(resto_order,self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
        line_ref = self.pool.get("resto.order.line")
        if view_type == 'form':
            ...
            if len(pref_ids)==0:
                ...
                xml_no_pref_1.set('class','oe_inline oe_resto_intro')
                ...
                xml_no_pref_3.text = _("Select a meal and put your order comments on the note.")
            ...
            #Else: the user already have preferences so we display them
                ...
                categories = {} #store the different categories of meals in preference
                count = 0
                for pref in preferences:
                    #For each preference
                    categories.setdefault(pref.meal_id.category_id.name, {})
                    #if this meal has already been added to the categories dictionnary
                    if pref.meal_id.id in categories[pref.meal_id.category_id.name]:
                        #we check if for the same meal the note has already been added
                        if pref.note not in categories[pref.meal_id.category_id.name][pref.meal_id.id]:
                            #if it's not the case then we add this to preferences
                            categories[pref.meal_id.category_id.name][pref.meal_id.id][pref.note] = pref
                    #if this meal is not in the dictionnay, we add it
                    else:
                        categories[pref.meal_id.category_id.name][pref.meal_id.id] = {}
                        categories[pref.meal_id.category_id.name][pref.meal_id.id][pref.note] = pref
                ...
                for key, value in categories.items():
                    ...
                    xml_pref_1.set('class', 'oe_resto_30pc')
                    ...
                    #NB: The note could also be ignored + we could fetch the preferences on the most ordered meals instead of the last ones...
                    sorted_values = {}
                    for val in value:
                    ...
                    for key, pref in sorted(sorted_values.iteritems(), key=lambda (k, v): (k, v), reverse=True):
                        ...
                        xml_pref_3.set('class','oe_resto_vignette')
                        xml_pref_1.append(xml_pref_3)

                        xml_pref_4 = etree.Element("span")
                        xml_pref_4.set('class','oe_resto_button')
                        ...
                        xml_pref_8.set('class','oe_resto_text')
                        xml_pref_8.text = escape(pref.meal_id.name)+str(" ")
                        xml_pref_7.append(xml_pref_8)

                        price = pref.meal_id.price or 0.0
    ...
    _columns = {
        ...
        'order_line_ids': fields.one2many('resto.order.line', 'order_id', 'meals', ondelete="cascade", readonly=True, states={'new':[('readonly', False)]}),
        'total': fields.function(_price_get, string="Total", store={
                 'resto.order.line': (_fetch_orders_from_lines, ['meal_id','order_id'], 20),
            }),
...
class resto_cashmove(osv.Model):
    """ 
    resto cashmove => order or payment 
    """
    _name = 'resto.cashmove'
    _description = 'resto cashmove'
    _columns = {
        ...
        'order_id': fields.many2one('resto.order.line', 'Order', ondelete='cascade'),
